            ...
            <>
              ...
              <div style={{ display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap", alignItems: "center" }}>
                <select
                  ...
                  style={selectStyle}
                >
                  {(Object.keys(TIER_LABEL) as QuestionTier[]).map((tr) => (
                    <option key={tr} value={tr}>{TIER_LABEL[tr]}</option>
                  ))}
                </select>
                {tierDraft === "check" && (
                  <select
...
function PreviewPanel({
  ...
  // 교사 수동 질문 추가 (설계서 항목 9) — 기본은 현재 마지막 페이지 연동 없이 check
