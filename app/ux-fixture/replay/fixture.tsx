...
export const REPLAY_GAMES = {
  puzzle: CulturePuzzle,
  halligalli: HalliGalli,
  yut: HoneyYut,
} as const;

export type ReplayKey = keyof typeof REPLAY_GAMES;
...
/**
 * 게임은 **마운트 뒤 클라이언트에서만** 그린다. Math.random 을 클라이언트에서만
 * 바꾸면 서버 렌더와 값이 달라져 하이드레이션이 어긋난다(지구본 퀴즈에서 실제로
 * 났던 D-02 와 같은 종류의 사고). 서버 HTML 에는 게임이 아예 들어가지 않는다.
 */
export default function ReplayFixture({ game, seed }: { game: ReplayKey; seed: number }) {
  ...
  const Active = REPLAY_GAMES[game];
